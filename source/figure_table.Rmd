...
#### summary stats quoted in the manuscript
...
cat(" Among these",length(unique(msm_long_t1_tmp$studyid)),"individuals were included in the main analyses that study the impact of clinical infection. These individuals contributed", nrow(msm_long_t1_tmp), "visits.")
...
# 
# msm_long_t1_tmp2 <- msm_long_t1_tmp %>% 
...
# fig 1c shows waning of vaccine protection
rbind(
...
# in the current season compared with those without confirmed infections.
...
rbind(
  ...
  # bring in output from IP weighted models that adjust for prior-season infection 
  quantile(output_msm_wanings_5pct_H1, c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_H3, c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_B, c(0.5,0.025,0.975))
) %>%
  data.frame() %>%
  `colnames<-`(c("odds_ratio","low","up")) %>%
...
#### Plot Fig 3: Impact of subclinical infection

```{r Fig3, fig.height=3.5, fig.width=7}

##' A function that outputs the fraction of non-repeat vaccinees who were subclinically infected in the prior season
...
plotdata <- plotdata %>% mutate(ratio=y/bseq, diff=y-bseq)



plotdata %>% filter(alpha1==0.3,y<1,y>0,bseq>0,bseq<1) %>%
  bind_rows(
    plotdata %>% filter(alpha1==0.5,y<1,y>0,bseq>0,bseq<1) 
  ) %>%
  bind_rows(
    plotdata %>% filter(alpha1==0.7,y<1,y>0,bseq>0,bseq<1) 
  ) %>%
  mutate(alpha1 = recode(as.character(alpha1), "0.3"="70%", "0.5"="50%", "0.7"="30%")) %>%
  ggplot(aes(x=bseq, y=y)) + 
  geom_path(aes(col=alpha1), size=1) +
  theme_bw() + 
  scale_color_manual(values = rev(c("#41ab5d","#74c476","#a1d99b")), name="effectiveness\nof  subclinical\ninfection") +
  xlab("fraction of repeat vaccinees who\nwere subclinically infected previous season") + 
  ylab("fraction of non-repeat vaccinees who\nwere subclinically infected previous season") +
  coord_cartesian(ylim=c(0, 0.5), xlim=c(0,0.5)) +
  geom_abline(slope=1,intercept=0, col="black", lty=3) + 
  theme(legend.position=c(0.75,0.3)) +
    theme(legend.key.height= unit(0.2, 'cm'),
        legend.key.width= unit(0.5, 'cm')) -> fig3_1


##' Function that outputs alpha*gamma_plus/gamma
...
##'
##' @return alpha*gamma_plus/gamma ... for computing delta_VE 
##' 
...
plotdata_img %>%
  mutate(VE = 0.5, 
         alpha = 0.7,
         gamma = 1-VE,
         delta_VE = gamma*(1-new_alpha/alpha) #  gamma - gamma+
    ) %>%
  ...
  theme_bw() + 
  scale_color_gradient2(name="increase in\noverall VE", low="#8c510a",mid="lightgrey",high="blue") +
  xlab("fraction of repeat vaccinees who\nwere subclinically infected previous season") + 
  ylab("fraction of non-repeat vaccinees who\nwere subclinically infected previous season") +
  coord_cartesian(ylim=c(0, 0.5), xlim=c(0,0.5)) +
  geom_abline(slope=1,intercept=0, col="black", lty=3) +
  theme(legend.position=c(0.8,0.35)) +
    theme(legend.key.height= unit(0.4, 'cm'),
        legend.key.width= unit(0.5, 'cm')) -> fig3_2


fig3_1 + fig3_2 + plot_annotation(tag_levels = 'A')

```

## Supplementary material

#### Stable1: Summary stat of FluVE data
...
#### Stable2: Summary stat of Marshfield data
...
#### SFig1.2: Seasonality by subtype - hidden
...
#### SFig3.1: Waning vaccine protection by subtype, season, and site.

```{r, sFig3.1, fig.height=15}
...
#### SFig3.2: Estimated effect of repeated vaccination by age group

```{r SFig3.2, fig.height=3.3, fig.width=6}
# BASE MODEL
output <- NULL
...
for (age_group_i in unique(dat_glm$age_cat_5)){
  for (var in c("RES_B", "RES_H1", "RES_H3")){
    ...
    model <- glm(as.formula(paste(var, "~ P_SEX + ANY_HR_PY + vac_group11 + vac_group10 + 
                                      ...
                                      as.factor(months_from_season_start) + as.factor(SEASON) +
                                  as.factor(SITE)")),
                 family="binomial",
                 data = dat_glm[dat_index,] %>% filter(age_cat_5 == age_group_i))
...
#### SFig3.3: Repeat vaccination effect by season and site
...
data.frame(list.rbind(ext_dat_H3)) %>%
  ...
  coord_cartesian(ylim=c(0.5, 2)) + 
  labs(color = "waning adjusted") + xlab("subtype") + ylab("adjusted odds ratio for clinical infection\ncomparing repeat with non-repeat vaccinees") +
  scale_linetype_manual(values=c(6,1)) +
  scale_shape_manual(values=c(1,16)) +
  theme_bw() + rremove("xlab") + 
...
#### SFig3.4: Impact of waning protection before and after excluding individuals vaccinated after the start of a season
...
#### sFig4.1: Odds of infection by years since the last clinical infection of the heterologous subtype

```{r SFig4.1, fig.width=5,fig.height=3}
...
#### SFig4.2: Regenerate figure 2 after excluding those who presented with acute respiratory illness but refused enrollment in the previous season

```{r SFig4.2, fig.height=10}
...
rbind(
  ...
  quantile(output_msm_wanings_refused_incl_H1,
           c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_refused_incl_H3,
           c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_refused_incl_B,
           c(0.5,0.025,0.975))
) %>%
  data.frame() %>%
  `colnames<-`(c("odds_ratio","low","up")) %>%
...
#### SFig4.3: aOR for clinical infection comparing repeat and non-repeat before and after adjusting for waning protection in the weighted outcome models

```{r SFig4.3, fig.width=4, fig.height=3}
rbind(
  #bring in data from models not adjusted for waning VE
  quantile(output_msm_nowanings_5pct_H1, c(0.5,0.025,0.975)),
  quantile(output_msm_nowanings_5pct_H3, c(0.5,0.025,0.975)),
  quantile(output_msm_nowanings_5pct_B,  c(0.5,0.025,0.975)),
  #bring in data from models adjusted for waning VE
  quantile(output_msm_wanings_5pct_H1, c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_H3, c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_B,  c(0.5,0.025,0.975))
  ...
  mutate(subtype = c("H1", "H3", "B",
                     "H1", "H3", "B"),
         model = rep(c("no waning adjustment", "waning adjustment"), each=3)) %>%
...
```
#### SFig4.4: Age-stratified version of Figure 2c

```{r SFig4.4, fig.width=4, fig.height=3}

rbind(
  # bring in output from base models not adjusted prior-season infection for those over 19yo
  get_repvac_OR_wrt01(output_glm_mf_o19$H1),
  get_repvac_OR_wrt01(output_glm_mf_o19$H3),
  get_repvac_OR_wrt01(output_glm_mf_o19$B),
  # bring in output from models that adjust for prior-season infection using inverse-probability weighting for those over 19yo
  quantile(output_msm_wanings_5pct_o19_H1,c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_o19_H3,c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_o19_B,c(0.5,0.025,0.975)),
  # bring in output from base models not adjusted prior-season infection for those under 19yo
  get_repvac_OR_wrt01(output_glm_mf_u19$H1),
  get_repvac_OR_wrt01(output_glm_mf_u19$H3),
  get_repvac_OR_wrt01(output_glm_mf_u19$B),
  # bring in output from models that adjust for prior-season infection using inverse-probability weighting for those under 19yo
  quantile(output_msm_wanings_5pct_u19_H1,c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_u19_H3,c(0.5,0.025,0.975)),
  quantile(output_msm_wanings_5pct_u19_B,c(0.5,0.025,0.975))
) %>%
  data.frame() %>%
  `colnames<-`(c("odds_ratio","low","up")) %>%
  mutate(model = c("glm","glm","glm",
         ...
         subtype = c("H1","H3","B",
                     ...
                     "H1","H3","B",
                     "H1","H3","B"),
         age = rep(c("over 19", "19 and under"), each=6)) %>%
  mutate(grouping = paste(age, model)) %>%
  ggplot(aes(x=subtype, y=odds_ratio, group=grouping, color=model)) +
  geom_point(position=position_dodge(width=0.5)) +
  geom_errorbar(aes(x=subtype, ymax=up, ymin=low, linetype=age),
                width=.1, position=position_dodge(width=0.5))+
  geom_hline(yintercept=1, lty=2, color="darkgrey") +
  labs(color = "model") + xlab("(sub)type") +
  ylab("adjusted odds ratio for infection\namong repeat vaccinees\ncompared with non-repeats") +
  ...
  scale_linetype_manual(values=c("twodash", "solid")) -> fig2c_agestratified


fig2c_agestratified
```

#### SFig 5.1: Test the infection block hypothesis

```{r Fig5.1, fig.height=3.2, fig.width=9}
...
#### SFig5.2: Similar to Fig 5.1 but using parameters that resemble values from high incidence settings

```{r SFig5.2, fig.height=3.2, fig.width=9}
...
#### SFig 5.3: Test the enhanced vaccine immunogenicity hypothesis

```{r SFig5.3, fig.height=3.2, fig.width=9}
...
#### SFig6.1: Prior-season clinical infection is unlikely to be an important mediator between prior-season vaccination and current-season clinical infection 
